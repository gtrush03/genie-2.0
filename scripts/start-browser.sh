...
ensure_dirs() {
  mkdir -p /tmp/genie-logs
  mkdir -p "$PROFILE"
}

kill_conflicting_chrome() {
  # Only kill Chrome processes that use OUR profile dir. Never touch George's main Chrome.
  local pids
...
cmd_logs() {
  tail -n 80 -f /tmp/genie-logs/chrome.out.log /tmp/genie-logs/chrome.err.log
}
